...
check("a02 起运 8岁0月11天22时（北京时口径：距惊蛰24天2时23分=34703分）",
      [qy["age"], qy["month"], qy["day"]] == [8, 0, 11], f"{qy}")
...
check("a06 交运时刻 2032-02-22 06:00（出生+8年0月11天22时）", dl["jiao_time"] == "2032-02-22 06:00", dl["jiao_time"])

# ---- 3. 同例女（阳年女 → 逆排） ----
r2 = B.compute(datetime(2024, 2, 10, 8, 0), 120.0, "女")
d2 = r2["dayun"]
check("b01 阳年女逆排（甲阳年×女）", d2["direction"] == "逆", d2["direction"])
check("b02 女起运 1岁10月17天18时（距上节立春 2024-02-04 16:27 间隔 5天15时33分=8133分）",
      [d2["qiyun"]["age"], d2["qiyun"]["month"], d2["qiyun"]["day"]] == [1, 10, 17], f"{d2['qiyun']}")
...
d3 = r3["dayun"]
check("c01 起运 9岁8月13天14时（北京时12:00 距惊蛰 29天2时43分=41923分）",
      [d3["qiyun"]["age"], d3["qiyun"]["month"], d3["qiyun"]["day"]] == [9, 8, 13], f"{d3['qiyun']}")
...
check("d02 起运 8岁3月23天6时（距上节大雪1989-12-07 11:21 间隔 24天22时39分=35919分）",
      [d4["qiyun"]["age"], d4["qiyun"]["month"], d4["qiyun"]["day"]] == [8, 3, 23], f"{d4['qiyun']}")
...
check("e02 起运 1岁6月2天18时（距下节小寒1990-01-05 22:33 间隔 4天12时33分=6513分）",
      [d5["qiyun"]["age"], d5["qiyun"]["month"], d5["qiyun"]["day"]] == [1, 6, 2], f"{d5['qiyun']}")
...
d6 = r6["dayun"]
check("f01 阳年女逆排+起运 3岁3月29天4时（距上节芒种2024-06-05 12:10 间隔 9天23时50分=14390分，北京时口径）",
...
# ---- 9b. 流月年份 base=交运年（bd-04 修正）：2024-11-15 男 起运 7岁1月26天10时 交运 2032-01-10 22:00，
#          出生+起运 进位跨年（出生年+起运岁=2031 ≠ 交运年 2032）→ 流月必须用交运年 2032 壬子年（壬寅起） ----
r7 = B.compute(datetime(2024, 11, 15, 12, 0), 120.0, "男")
check("h03 流月年份进位修正（2024-11-15 男：交运 2032-01-10 22:00，非出生年+起运岁 2031）：12 节月 2032 壬子年壬寅起",
...
# ---- 12. 与 m1 内核同轨：起运节=北京时域最近节（逆排 2024-02-10 女 → 立春；2026-09-15 qiyun 修复后同域，真太阳时仅日/时柱） ----
check("k01 逆排起运节=立春 2024-02-04 16:27（北京时域最近上节）",
      d2["jie_time"] == "2024-02-04 16:27", d2["jie_time"])

# ---- 13. F1 判界时制回归（2026-09-15：节气判界两侧统一北京时域，禁与真太阳时混比） ----
# 1974 立春=1974-02-04 13:00（北京时）；乌鲁木齐 87.6° 真太阳时慢约 2h23m42s（经度差 2h09m36s + EOT 约 14m），
# 修复前此处年/月柱误用真太阳时（14:30→12:06:18、12:30→10:06:18）与节气表比 → 判癸丑/乙丑。
r8 = B.compute(datetime(1974, 2, 4, 14, 30), 87.6, "女")  # 北京时 14:30 ≥ 立春 13:00
check("m01 判界时制：乌市 1974-02-04 14:30（北京时≥立春13:00）→ 年甲寅月丙寅（修复前误癸丑/乙丑）",
      gz(r8, "year") == "甲寅" and gz(r8, "month") == "丙寅", f"{gz(r8, 'year')} {gz(r8, 'month')}")
r9 = B.compute(datetime(1974, 2, 4, 12, 30), 87.6, "女")  # 北京时 12:30 < 立春 13:00
check("m02 判界时制：同上 12:30（北京时<立春13:00）→ 年癸丑月乙丑（界前不误判）",
      gz(r9, "year") == "癸丑" and gz(r9, "month") == "乙丑", f"{gz(r9, 'year')} {gz(r9, 'month')}")
